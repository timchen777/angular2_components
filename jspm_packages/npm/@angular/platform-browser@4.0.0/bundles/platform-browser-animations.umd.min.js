...
!function(global, factory) {
    ...
    d.prototype = null === b ? Object.create(b) : (__.prototype = b.prototype, new __);
      ...
      AnimationRendererFactory = function() {
        function AnimationRendererFactory(delegate, _engine, _zone) {
          this.delegate = delegate, this._engine = _engine, this._zone = _zone;
        }
        return AnimationRendererFactory.prototype.createRenderer = function(hostElement, type) {
          ...
          var namespaceId = type.id,
              animationTriggers = type.data.animation;
          ...
          }), new AnimationRenderer(delegate, this._engine, this._zone, namespaceId);
  ...
  var AnimationRenderer = function() {
    function AnimationRenderer(delegate, _engine, _zone, _namespaceId) {
      this.delegate = delegate, this._engine = _engine, this._zone = _zone, this._namespaceId = _namespaceId, this.destroyNode = null, this._flushPromise = null, this.destroyNode = this.delegate.destroyNode ? function(n) {
        return delegate.destroyNode(n);
      } : null;
    }
    return Object.defineProperty(AnimationRenderer.prototype, "data", {
      get: function() {
      ...
      this._engine.onRemove(oldChild, function() {
        _this.delegate.parentNode(oldChild) && _this.delegate.removeChild(parent, oldChild);
      }), this._queueFlush();
    }, AnimationRenderer.prototype.setProperty = function(el, name, value) {
      "@" == name.charAt(0) ? (this._engine.setProperty(el, namespaceify(this._namespaceId, name.substr(1)), value), this._queueFlush()) : this.delegate.setProperty(el, name, value);
    }, AnimationRenderer.prototype.listen = function(target, eventName, callback) {
      var _this = this;
      if ("@" == eventName.charAt(0)) {
        var element = resolveElementFromTarget(target),
            _a = parseTriggerCallbackName(eventName.substr(1)),
            name = _a[0],
            phase = _a[1];
        return this._engine.listen(element, namespaceify(this._namespaceId, name), phase, function(event) {
          var e = event;
          e.triggerName && (e.triggerName = deNamespaceify(_this._namespaceId, e.triggerName)), _this._zone.run(function() {
      ...
      var _this = this;
      this._flushPromise || this._zone.runOutsideAngular(function() {
        _this._flushPromise = Promise.resolve(null).then(function() {
          _this._flushPromise = null, _this._engine.flush();
        });
      });
    }, AnimationRenderer;
  }(),
